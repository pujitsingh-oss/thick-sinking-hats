...
const model = JSON.parse(fs.readFileSync(path.join(__dirname,'../models/attrition_model.json'),'utf8'));
const features = JSON.parse(fs.readFileSync(path.join(__dirname,'../models/latest_features.json'),'utf8'));
...
function scoreRow(row){
  ...
  for(const [k,coef] of Object.entries(model.features)){
    ...
    z += coef * x;
    if (coef !== 0 && x !== 0) reasons.push({feature:k, impact: Number((coef*x).toFixed(2))});
  }
...
exports.handler = async (event)=>{
  ...
  return { statusCode:200, body: JSON.stringify({ as_of:new Date().toISOString().slice(0,10), team_id, members: scored }) };
};
